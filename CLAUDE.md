...
- **Linter/formatter**: Biome (editor-only, not run in CI); Tailwind class sorting on; CSS skipped (Tailwind v4 directives unsupported).
- **Squash-merge only** (2026-06-27). Every PR lands on `main` as one squashed commit; PR title = the conventional-commit subject (GitHub appends `(#NN)`), PR description = the commit body. PR-title format is CI-checked (`.github/workflows/lint-pr-title.yml` → `amannn/action-semantic-pull-request@v6`, pinned + Dependabot-updated) — the repo's one CI gate; Biome/tests/build stay local. See Non-negotiables → Conventional Commits / squash-merge.
