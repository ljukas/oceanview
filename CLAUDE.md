...
- **Squash-merge only** (2026-06-27). Every PR lands on `main` as one squashed commit; PR title = the conventional-commit subject (GitHub appends `(#NN)`), PR description = the commit body. PR-title format is CI-checked (`.github/workflows/lint-pr-title.yml` → `amannn/action-semantic-pull-request@v6`, pinned + Dependabot-updated), alongside `ci.yml` (Biome/audit/build/test). No branch protection yet, so checks report but don't block merge. See Non-negotiables → Conventional Commits / squash-merge.
