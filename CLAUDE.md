...
- **Visual identity / design language** (2026-06-17, design only — not yet built). "Quiet nautical confidence": deploy the sidebar `variant="inset"` (rounded content panel, sidebar bg wraps) + a shared centered `PageContainer`; self-hosted **Cabinet Grotesk** headings over **Switzer** body (both Fontshare ITF-FFL variable woff2; Geist dropped) with Linear-style tuning (tight tracking, optical sizing, tabular numerals); one `--brand` nautical-blue token applied only to login/empty washes + a logo mark (`--primary` stays neutral); slower, `prefers-reduced-motion`-aware overlay motion. Plans in `docs/plans/redesign-2026-06/`. See ADR-0015.
