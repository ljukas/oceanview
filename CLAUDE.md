...
- **The SSE stream is gated on tab activity** (2026-08-06). `useActivityGate()` (`src/hooks/useActivityGate.ts`, over the pure reducer `src/utils/activityGate.ts`) feeds `useRealtimeSync`'s effect deps: a tab hidden 60s, or visible but idle 30 min, drops its connection and reopens on visibility/interaction. Reuses the existing `controller.abort()` + `backOff` path — no server change. **Why**: an open SSE stream is an in-flight request, and Vercel bills Fluid Provisioned Memory for an instance's whole lifetime — one tab open 24/7 is ~1,460 GB-hrs/month against a 360 GB-hr Hobby allowance, which soft-blocked the account on 2026-08-05 (`fluidDuration` overage). Gaps are now routine, so `useRealtimeSync` replays every kind in `ALL_EVENT_KINDS` on a post-gate reconnect — `refetchOnWindowFocus` only covers the hidden→visible path (its `focusManager` listens solely to `visibilitychange`), not idle-while-visible. See ADR-0011's amendment.
- **Presence**: online status via an in-process refcounted `presence` effect on the SSE connection lifecycle; `presence.changed` (no ids) → `listOnline()` refetch. Single-instance assumption shared with ADR-0004. Since 2026-08-06 "online" means **open and in use** — the activity gate above means a backgrounded or idle tab reads as offline; the binary refcount data model is unchanged and a three-state away/idle UI remains out of scope. See ADR-0011.
